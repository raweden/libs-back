...
_CGRectRepr(CGRect rect)
{
    return [NSString stringWithFormat:@"(%g,%g,%g,%g)", rect.origin.x, rect.origin.y, rect.size.width, rect.size.height];
}

static inline CGRect
_CGRectFromNSRect(NSRect nsrect)
{
    return CGRectMake(nsrect.origin.x, nsrect.origin.y, nsrect.size.width, nsrect.size.height);
}

static inline NSPoint
_NSPointFromCGPoint(CGPoint cgpoint)
{
...
{
    NSDebugLLog(@"OpalGState", @"%p (%@): %s - %g %g", self, [self class], __PRETTY_FUNCTION__, theOffset.x,
                theOffset.y);
    CGContextRef cgctx = CGCTX;

    if (cgctx) {
        OPContextSetCairoDeviceOffset(cgctx, -theOffset.x, theOffset.y - [_opalSurface size].height);
    }
...
- (void)setColor:(device_color_t *)color state:(color_state_t)cState
{
    ...
    if (cgctx) {
        ...
        if (color->space == gray_colorspace) {
            ...
            if (cState & COLOR_STROKE) {
                CGContextSetRGBStrokeColor(cgctx, color->field[0], color->field[1], color->field[2],
                                           color->field[AINDEX]);
            }
            if (cState & COLOR_FILL) {
                CGContextSetRGBFillColor(cgctx, color->field[0], color->field[1], color->field[2],
                                         color->field[AINDEX]);
            }
        } else if (color->space == cmyk_colorspace) {
            if (cState & COLOR_STROKE) {
                CGContextSetCMYKStrokeColor(cgctx, color->field[0], color->field[1], color->field[2],
                                            color->field[3], color->field[AINDEX]);
            }
            if (cState & COLOR_FILL) {
                CGContextSetCMYKFillColor(cgctx, color->field[0], color->field[1], color->field[2], color->field[3],
                                          color->field[AINDEX]);
            }
        }
    }
...
- (void)GSSetFont:(GSFontInfo *)fontref
{
    ...
    if (cgctx) {
        ...
        CGAffineTransform cgAT =
            CGAffineTransformMake(matrix[0], matrix[1], matrix[2], matrix[3], matrix[4], matrix[5]);
...
- (void)DPSsetdash:(const CGFloat *)pat:(NSInteger)size:(CGFloat)dashOffset
{
    NSDebugLLog(@"OpalGState", @"%p (%@): %s", self, [self class], __PRETTY_FUNCTION__);
...
{
    NSDebugLLog(@"OpalGState", @"%p (%@): %s - %g %g %g %g %g %g", self, [self class], __PRETTY_FUNCTION__, m[0],
                m[1], m[2], m[3], m[4], m[5]);
    CGContextRef cgctx = CGCTX;

    if (cgctx) {
        CGContextConcatCTM(cgctx, CGAffineTransformMake(m[0], m[1], m[2], m[3], m[4], m[5]));
    }
    [super DPSconcat:m];
}
...
{
    NSDebugLLog(@"OpalGState", @"%p (%@): %s - rect %g %g %g %g", self, [self class], __PRETTY_FUNCTION__, x, y, w,
                h);
    CGContextRef cgctx = CGCTX;
        ...
        for (i = 0; i < count; i++) {
            ...
            switch (type) {
                ...
                case NSCurveToBezierPathElement:
                    CGContextAddCurveToPoint(cgctx, points[0].x, points[0].y, points[1].x, points[1].y, points[2].x,
                                             points[2].y);
                    break;
		 ...
		 : (const unsigned char *const[5])data
{
    ...
    if (cgctx) {
        ...
        NSData *nsData = [NSData dataWithBytesNoCopy:(void *)*data length:pixelsHigh * bytesPerRow freeWhenDone:NO];
        CGDataProviderRef dataProvider = CGDataProviderCreateWithCFData(nsData);
        CGImageRef img = CGImageCreate(pixelsWide, pixelsHigh, bitsPerSample, bitsPerPixel, bytesPerRow, colorSpace,
                                       kCGBitmapByteOrder32Big | (hasAlpha ? kCGImageAlphaPremultipliedLast : 0),
                                       dataProvider, NULL, /* const CGFloat decode[] is what? */
          ...
          destCGContext:(CGContextRef)destCGContext
{
    ...
    NSDebugLLog(@"OpalGState", @"%p (%@): %s - from %@ of gstate %p (cgctx %p) to %@ of %p (cgctx %p)", self,
                [self class], __PRETTY_FUNCTION__, NSStringFromRect(srcRect), source, [source CGContext],
                NSStringFromPoint(destPoint), self, [self CGContext]);
    ...
    CGRect destCGRect = CGRectMake(destPoint.x, destPoint.y, srcRect.size.width, srcRect.size.height);
    NSDebugLLog(@"OpalGState", @"Source cgctx: %p, self: %p - from %@ to %@ with ctm %@", [source CGContext], self,
                _CGRectRepr(srcCGRect), _CGRectRepr(destCGRect), [self GSCurrentCTM]);
    ...
    OPContextSetCairoDeviceOffset(CGCTX, -offset.x, offset.y - [_opalSurface size].height);
               ...
               fraction:(CGFloat)delta
{
    ...
    for (i = 0; i < 1; i++)  // not drawing into x11cgctx after all.
    {
        ...
        [self compositeGState:source fromRect:srcRect toPoint:destPoint op:op fraction:delta destCGContext:ctx];
    }
          ...
          fraction:(CGFloat)delta
{
    ...
    for (i = 0; i < 1; i++) {
        ...
        [self drawGState:source fromRect:srcRect toPoint:destPoint op:op fraction:delta destCGContext:ctx];
    }
}
...
- (void)compositerect:(NSRect)aRect op:(NSCompositingOperation)op
{
    ...
    if (cgctx) {
        ...
        OPContextSetIdentityCTM(cgctx);
        // FIXME: Set operator
        CGContextFillRect(cgctx, CGRectMake(aRect.origin.x, [_opalSurface size].height - aRect.origin.y,
...
@implementation
OpalGState (InitializationMethods)
 ...
 **/
- (void)GSSetSurface:(OpalSurface *)opalSurface:(int)x:(int)y
{
    NSDebugLLog(@"OpalGState", @"%p (%@): %s - %@ %d %d", self, [self class], __PRETTY_FUNCTION__, opalSurface, x,
                y);
...
- (void)GSCurrentSurface:(OpalSurface **)surface:(int *)x:(int *)y
{
    NSDebugLLog(@"OpalGState", @"%p (%@): %s", self, [self class], __PRETTY_FUNCTION__);
