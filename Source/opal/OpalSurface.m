...
static CGContextRef
createCGBitmapContext(int pixelsWide, int pixelsHigh)
{
...
- (void)createCGContextsWithSuppliedBackingContext:(CGContextRef)ctx
{
    ...
    if (_x11CGContext || _backingCGContext) {
        NSLog(@"FIXME: Replacement of OpalSurface %p's CGContexts (x11=%p,backing=%p) without transfer of gstate",
              self, _x11CGContext, _backingCGContext);
    }
...
#warning All windows have to be doublebuffered
    ...
    NSDebugLLog(@"OpalSurface", @"Created CGContexts: X11=%p, backing=%p, width=%d height=%d", _x11CGContext,
                _backingCGContext, pixelsWide, pixelsHigh);
}
...
- (void)handleExposeRect:(NSRect)rect
{
    ...
    cgRect =
        CGRectIntersection(cgRect, CGRectMake(0, 0, CGImageGetWidth(backingImage), CGImageGetHeight(backingImage)));
    ...
    NSDebugLLog(@"OpalSurface", @" ... actually from %@ to %@", NSStringFromRect(*(NSRect *)&subimageCGRect),
                NSStringFromRect(*(NSRect *)&cgRect));
...
#warning Opal bug: cannot properly save subimage created with CGImageCreateWithImageInRect()
    ...
    NSString *path =
        [NSString stringWithFormat:@"%@%dx%d.png", prefix, CGImageGetWidth(img), CGImageGetHeight(img)];
    CFURLRef fileUrl = (CFURLRef)[[NSURL fileURLWithPath:path] retain];
    NSLog(@"FileURL %@", fileUrl);
    // CGImageDestinationRef outfile = CGImageDestinationCreateWithURL(fileUrl, @"public.jpeg"/*kUTTypeJPEG*/, 1,
    // NULL);
...
- (NSSize)size
{
    return NSMakeSize(CGBitmapContextGetWidth(_backingCGContext), CGBitmapContextGetHeight(_backingCGContext));
}
