...
@interface
GSFontInfo (experimental_glyph_printing_extension)
...
#endif
        if (!gstream) {
            NSDebugLLog(@"GSContext", @"%@: Could not open printer file %@", DPSinvalidfileaccess, path);
            return nil;
        }
    } else {
        NSDebugLLog(@"GSContext", @"%@: No stream file specified", DPSconfigurationerror);
        DESTROY(self);
        return nil;
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        switch (type) {
            ...
            case NSCurveToBezierPathElement:
                [self DPScurveto:pts[0].x:pts[0].y:pts[1].x:pts[1].y:pts[2].x:pts[2].y];
                break;
...
{
    [self DPScomposite:NSMinX(srcRect):NSMinY(srcRect):NSWidth(srcRect):NSHeight(srcRect):gstateNum:aPoint.x:aPoint.y:op];
}
...
- (void)GSDrawImage:(NSRect)rect:(void *)imageref
{
    ...
    if ([image isKindOfClass:[NSBitmapImageRep class]]) {
        ...
        [self NSDrawBitmap:rect:[image pixelsWide]:[image pixelsHigh]:[image bitsPerSample]:[image samplesPerPixel
        ]:[image bitsPerPixel]:[image bytesPerRow]:[image isPlanar]:[image hasAlpha]:[image colorSpaceName
                    ...
                    :(const unsigned char *const[5])data
{
    ...
    if (bitsPerSample == 0)
        bitsPerSample = 8;
    bytes = (bitsPerSample * pixelsWide * pixelsHigh + 7) / 8;
    if (bytes * samplesPerPixel != bytesPerRow * pixelsHigh) {
        NSLog(@"Image Rendering Error: Dodgy bytesPerRow value %d", (int)bytesPerRow);
        NSLog(@"   pixelsHigh=%d, bytes=%d, samplesPerPixel=%d", (int)bytesPerRow, (int)pixelsHigh, (int)bytes);
        return;
    }
    if (hasAlpha)
    ...
    if (samplesPerPixel > 1) {
        ...
        fprintf(gstream, "%d %d %d [%d 0 0 %d 0 %d]\n", (int)pixelsWide, (int)pixelsHigh, (int)bitsPerSample,
                (int)pixelsWide, (flipped) ? (int)pixelsHigh : (int)-pixelsHigh, (int)pixelsHigh);
