...
{
    [self compositeGState:source fromRect:aRect toPoint:aPoint op:op fraction:1.0];
}

// This is only a fall back, the method should not be called any more.
- (void)dissolveGState:(GSGState *)source fromRect:(NSRect)aRect toPoint:(NSPoint)aPoint delta:(CGFloat)delta
{
    [self compositeGState:source fromRect:aRect toPoint:aPoint op:NSCompositeSourceOver fraction:delta];
}
...
- (void)GSSetFillColor:(const CGFloat *)values
{
    ...
    if ((fillColorS == nil) ||
        ((color = [NSColor colorWithColorSpace:fillColorS components:values
        ...
        gsMakeColor(&dcolor, rgb_colorspace, values[0], values[1], values[2], values[3]);
...
- (void)GSSetStrokeColor:(const CGFloat *)values
{
    ...
    if ((strokeColorS == nil) ||
        ((color = [NSColor colorWithColorSpace:strokeColorS components:values
         ...
         isRelative:(BOOL)relative;
{
    ...
    */
    uch = NULL;
    ulen = 0;
    GSToUnicode(&uch, &ulen, (const unsigned char *)s, strlen(s), [font mostCompatibleStringEncoding],
    ...
    for (i = 0; i < ulen; i++) {
        ...
        if (relative == YES) {
            ...
            advancement = [ctm transformSize:NSMakeSize(advancement.width, [font ascender])];
...
- (void)DPSashow:(CGFloat)x:(CGFloat)y:(const char *)s
{
    ...
    arr[0] = x;
    arr[1] = y;
    [self _showString:s xCharAdj:0 yCharAdj:0 char:0 adjArray:arr arrType:show_delta isRelative:YES];
}
...
- (void)DPSawidthshow:(CGFloat)cx:(CGFloat)cy:(int)c:(CGFloat)ax:(CGFloat)ay:(const char *)s
{
    ...
    arr[0] = ax;
    arr[1] = ay;
    [self _showString:s xCharAdj:cx yCharAdj:cy char:c adjArray:arr arrType:show_delta isRelative:YES];
}
...
- (void)DPScharpath:(const char *)s:(int)count
{
    ...
    CHECK_PATH;
    [font appendBezierPathWithGlyphs:glBuf count:count toBezierPath:path];
}
...
- (void)appendBezierPathWithPackedGlyphs:(const char *)packedGlyphs path:(NSBezierPath *)aPath
{
    ...
    for (i = 0; i < count; i++) {
        switch (packing) {
            ...
            case NSFourByteGlyphPacking:
                ...
                c = packedGlyphs[j++];
                d = packedGlyphs[j++];
                glBuf[i] = (NSGlyph)((a << 24) | (b << 16) | (c << 8) | d);
                break;
    ...
    [font appendBezierPathWithGlyphs:glBuf count:count toBezierPath:aPath];
}
...
- (void)DPSwidthshow:(CGFloat)x:(CGFloat)y:(int)c:(const char *)s
{
    ...
    arr[0] = 0;
    arr[1] = 0;
    [self _showString:s xCharAdj:x yCharAdj:y char:c adjArray:arr arrType:show_delta isRelative:YES];
}

- (void)DPSxshow:(const char *)s:(const CGFloat *)numarray:(int)size
{
    [self _showString:s xCharAdj:0 yCharAdj:0 char:0 adjArray:numarray arrType:show_array_x isRelative:NO];
}

- (void)DPSxyshow:(const char *)s:(const CGFloat *)numarray:(int)size
{
    [self _showString:s xCharAdj:0 yCharAdj:0 char:0 adjArray:numarray arrType:show_array_xy isRelative:NO];
}

- (void)DPSyshow:(const char *)s:(const CGFloat *)numarray:(int)size
{
    [self _showString:s xCharAdj:0 yCharAdj:0 char:0 adjArray:numarray arrType:show_array_y isRelative:NO];
}
...
- (void)DPSarc:(CGFloat)x:(CGFloat)y:(CGFloat)r:(CGFloat)angle1:(CGFloat)angle2
{
    ...
    [newPath appendBezierPathWithArcWithCenter:NSMakePoint(x, y) radius:r startAngle:angle1 endAngle:angle2
                                     clockwise:NO];
...
- (void)DPSarct:(CGFloat)x1:(CGFloat)y1:(CGFloat)x2:(CGFloat)y2:(CGFloat)r
{
    ...
    [newPath appendBezierPathWithArcFromPoint:NSMakePoint(x1, y1) toPoint:NSMakePoint(x2, y2) radius:r];
    [newPath transformUsingAffineTransform:ctm];
    CHECK_PATH;
    [path appendBezierPath:newPath];
    RELEASE(newPath);
...
- (void)DPSrcurveto:(CGFloat)x1:(CGFloat)y1:(CGFloat)x2:(CGFloat)y2:(CGFloat)x3:(CGFloat)y3
{
    ...
    CHECK_PATH;
    [path relativeCurveToPoint:p3 controlPoint1:p1 controlPoint2:p2];
}
...
- (void)GSRectClipList:(const NSRect *)rects:(int)count
{
    ...
    */
    ...
    [self DPSrectclip:NSMinX(union_rect):NSMinY(union_rect):NSWidth(union_rect):NSHeight(union_rect)];
}
...
- (void)GSRectFillList:(const NSRect *)rects:(int)count
{
    int i;
    for (i = 0; i < count; i++)
        [self DPSrectfill:NSMinX(rects[i]):NSMinY(rects[i]):NSWidth(rects[i]):NSHeight(rects[i])];
}
...
- (void)DPSimage:(NSAffineTransform *)
          matrix:(NSInteger)pixelsWide
...
- (void)DPSshfill:(NSDictionary *)shader
{
    ...
    [self DPSimage:matrix:iwidth:iheight:8:4:32:4 *
            iwidth:NO:YES:NSDeviceRGBColorSpace:(const unsigned char **)&data];
    free(data);
...
- (void)_fillRect:(NSRect)rect withPattern:(NSImage *)color_pattern
{
    ...
    startPoint = NSMakePoint(floor((NSMinX(rect) - patternPhase.x) / size.width) * size.width + patternPhase.x,
                             floor((NSMinY(rect) - patternPhase.y) / size.height) * size.height + patternPhase.y);
    ...
    for (point.y = startPoint.y; point.y < endPoint.y; point.y += size.height) {
        for (point.x = startPoint.x; point.x < endPoint.x; point.x += size.width) {
            [color_pattern compositeToPoint:[ictm transformPoint:point] operation:NSCompositeSourceOver];
        }
    }
    RELEASE(ictm);
}
