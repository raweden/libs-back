...
- (id)initWith:(NSDictionary *)d
{
    ...
    j = (j + 7) / 8;
    if ([data length] < j) {
        NSDebugLLog(@"GSFunction", @"Need %i bytes of data, DataSource only has %lu bytes.", j,
...
- (void)eval:(double *)inValues:(double *)outValues;
{
    ...
    if (sample[0] != sample_index[0] || sample[1] != sample_index[1]) {
        ...
        for (i = 0; i < 3; i++) {
            sample_cache[0][i] = [self getsample:sample[0] + sample[1] * size[0]:i];
            if (sample[0] + 1 < size[0])
                sample_cache[1][i] = [self getsample:sample[0] + 1 + sample[1] * size[0]:i];
            if (sample[1] + 1 < size[1])
                sample_cache[2][i] = [self getsample:sample[0] + (sample[1] + 1) * size[0]:i];
            if (sample[0] + 1 < size[0] && sample[1] + 1 < size[1])
                sample_cache[3][i] = [self getsample:sample[0] + 1 + (sample[1] + 1) * size[0]:i];
        }
    }
