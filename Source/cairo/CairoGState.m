...
doubleFromUserSpace(NSAffineTransform *ctm, CGFloat d)
{
    if (ctm) {
        NSSize s = {d, d};

        s = [ctm transformSize:s];
        d = (((s.width > 0.0) ? s.width : -s.width) + ((s.height > 0.0) ? s.height : -s.height)) / 2;
    }
    return d;
}

static inline CGFloat
floatToUserSpace(NSAffineTransform *ctm, double d)
{
    if (ctm) {
            ...
            if (status == CAIRO_STATUS_SUCCESS) {
                ...
                if (cairo_version() >= CAIRO_VERSION_ENCODE(1, 6, 0)) {
                    for (i = 0; i < clip_rects->num_rectangles; i++) {
                        ...
                        cairo_rectangle(copy->_ct, rect.x, rect.y, rect.width, rect.height);
                        cairo_clip(copy->_ct);
                    }
                } else {
                    for (i = 0; i < clip_rects->num_rectangles; i++) {
                        ...
                        cairo_rectangle(copy->_ct, rect.x,
                                        ...
                                        rect.y + 2 * (offset.y - size.height), rect.width, rect.height);
                        cairo_clip(copy->_ct);
                    }
                }
...
- (void)setOffset:(NSPoint)theOffset
{
    if (_surface != nil) {
        ...
        cairo_surface_set_device_offset([_surface surface], -theOffset.x, theOffset.y - size.height);
    }
...
- (void)DPSshow:(const char *)s
{
    if (_ct) {
        ...
        [self _setPoint];
        cairo_save(_ct);
        cairo_matrix_init(&local_matrix, matrix.m11, 0, 0, matrix.m22, 0, 0);
...
- (void)GSShowGlyphsWithAdvances:(const NSGlyph *)glyphs:(const NSSize *)advances:(size_t)length
{
    ...
    if (_ct) {
        ...
        cairo_save(_ct);
        cairo_matrix_init(&local_matrix, matrix.m11, matrix.m12, matrix.m21, matrix.m22, 0, 0);
        cairo_transform(_ct, &local_matrix);
        // Undo the
        cairo_matrix_init_scale(&local_matrix, 1, -1);
        cairo_matrix_translate(&local_matrix, 0, -[_surface size].height);
        ...
        [(CairoFontInfo *)font drawGlyphs:glyphs length:length on:_ct];
    ...
    for (i = 0; i < count; i++) {
        ...
        switch (type) {
            ...
            case NSCurveToBezierPathElement:
                cairo_curve_to(_ct, points[0].x, points[0].y, points[1].x, points[1].y, points[2].x, points[2].y);
                break;
...
- (NSDictionary *)GSReadRect:(NSRect)r
{
    ...
    if (!baseDict) {
        ...
        baseDict = [[NSDictionary
            dictionaryWithObjectsAndKeys:NSDeviceRGBColorSpace, @"ColorSpace", [NSNumber numberWithUnsignedInt:8],
                                         @"BitsPerSample", [NSNumber numberWithUnsignedInt:32], @"Depth",
                                         [NSNumber numberWithUnsignedInt:4], @"SamplesPerPixel",
                                         [NSNumber numberWithUnsignedInt:1], @"HasAlpha", nil] retain];
#else
        baseDict = @{
    ...
    [matrix translateXBy:-r.origin.x - offset.x yBy:r.origin.y + NSHeight(r) - offset.y];
                ...
                :(const unsigned char *const[5])data
{
    ...
    surface = cairo_image_surface_create_for_data((void *)reformattedData, format, pixelsWide, pixelsHigh,
    ...
    cairo_matrix_init(&local_matrix, tstruct.m11, tstruct.m12, tstruct.m21, tstruct.m22, tstruct.tX, tstruct.tY);
    cairo_transform(_ct, &local_matrix);
...

    {
        ...
        cairo_pattern_set_filter(cpattern, cairoFilterFromNSImageInterpolation([drawcontext imageInterpolation]));
        if (cairo_version() >= CAIRO_VERSION_ENCODE(1, 6, 0)) {
               ...
               fraction:(CGFloat)delta
{
    ...
    // NSLog(@"Composite surface %p source size %@ target size %@", self->_surface, NSStringFromSize([self->_surface
    // size]), NSStringFromSize([source->_surface size]));
    ...
    // NSLog(@"Point in base adjusted %@", NSStringFromPoint(NSMakePoint(destPointInBase.x - dx, destPointInBase.y -
    // dy)));
    ...
    if ((cairo_version() >= CAIRO_VERSION_ENCODE(1, 8, 0)) && (cairo_version() <= CAIRO_VERSION_ENCODE(1, 13, 0))) {
          ...
          fraction:(CGFloat)delta
{
    ...
    NSDebugMLLog(@"CairoGState", @"source: %@ fromRect: %@ toPoint: %@\n", source, NSStringFromRect(aRect),
                 NSStringFromPoint(aPoint));

    if (!_ct || !source->_ct) {
        NSLog(@"WARNING: -drawGState called with a NULL target context (%p) or source context (%p)", _ct,
              source->_ct);
        return;
    }
    ...
    if (_ct && savedClip) {
        ...
        if (cairo_version() >= CAIRO_VERSION_ENCODE(1, 6, 0)) {
            for (i = 0; i < clip_rects->num_rectangles; i++) {
                ...
                cairo_rectangle(_ct, rect.x, rect.y, rect.width, rect.height);
                cairo_clip(_ct);
            }
        } else {
            for (i = 0; i < clip_rects->num_rectangles; i++) {
             ...
             options:(NSUInteger)options
{
    if (_ct) {
        ...
        cairo_pattern_t *cpattern =
            cairo_pattern_create_radial(startP.x, startP.y, doubleFromUserSpace(ctm, startRadius), endP.x, endP.y,
        ...
        for (i = 0; i < stops; i++) {
            ...
            cairo_pattern_add_color_stop_rgba(cpattern, location, red, green, blue, alpha);
        }
             ...
             options:(NSUInteger)options
{
    if (_ct) {
        ...
        cairo_pattern_t *cpattern = cairo_pattern_create_linear(startP.x, startP.y, endP.x, endP.y);
