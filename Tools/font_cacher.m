...
- (NSString *)creationNameFromParts:(NSArray *)parts
{
    ...
    creationName =
        [NSString stringWithFormat:@"-%@-%@-%@-%@-%@-%@-%@-%@-%@-%@-%@-%@-%@-%@", @"*", [parts objectAtIndex:2],
                                   [parts objectAtIndex:3], [parts objectAtIndex:4], [parts objectAtIndex:5],
                                   ...
                                   [parts objectAtIndex:13], [parts objectAtIndex:14]];
...
- (NSString *)getPathFor:(NSString *)display
{
    ...
    if ([mgr fileExistsAtPath:path] == NO) {
        NSError *err;
        BOOL r;
        r = [mgr createDirectoryAtPath:path withIntermediateDirectories:YES attributes:nil error:&err];
        if (r == NO)
...
fontDefSorter(id e1, id e2, void *context)
{
...
- (void)sortResults
{
    ...
    while ((key = [enumerator nextObject])) {
        ...
        while ((name != nil) && (creationName = [creationDictionary objectForKey:name]) == nil) {
...
main(int argc, char **argv, char **env)
{
    ...
    if ([args containsObject:@"--version"]) {
        NSLog(@"%@ version %s", [[args objectAtIndex:0] lastPathComponent], makever(GNUSTEP_VERSION));
        RELEASE(pool);
        return 0;
    }
