 ...
 */
static char *atom_names[] = {"CHARACTER_POSITION", "CLIENT_WINDOW", "HOST_NAME", "HOSTNAME", "LENGTH",
                             "LIST_LENGTH", "NAME", "OWNER_OS", "SPAN", "TARGETS", "TIMESTAMP", "USER", "TEXT",
                             "NULL", "FILE_NAME", "CLIPBOARD", "UTF8_STRING", "MULTIPLE", "COMPOUND_TEXT", "INCR",

                             // some MIME types
                             "text/plain", "text/uri-list", "application/postscript", "text/tab-separated-values",
                             "text/richtext", "image/tiff", "application/octet-stream",
                             "application/x-rootwindow-drop", "application/richtext", "text/rtf", "text/html",
                             "application/xhtml+xml", "image/png", "image/svg", "application/rtf", "text/richtext"};
...
- (BOOL)xSendData:(unsigned char *)data
           format:(int)format
            items:(int)numItems
             type:(Atom)xType
               to:(Window)window
...
+ (BOOL)initializePasteboard
{
    ...
    ownByO = NSCreateMapTable(NSObjectMapKeyCallBacks, NSNonOwnedPointerMapValueCallBacks, 0);
    ownByX = NSCreateMapTable(NSIntMapKeyCallBacks, NSNonOwnedPointerMapValueCallBacks, 0);
    ...
    XInternAtoms(xDisplay, atom_names, sizeof(atom_names) / sizeof(char *), False, atoms);
#else
    {
        int atomCount;
    ...
    xAppWin = XCreateSimpleWindow(xDisplay, xRootWin, 0, 0, 100, 100, 1, 1, 0L);
    /*
    ...
    [l addEvent:(void *)(gsaddr)desc type:ET_RDESC watcher:(id<RunLoopEvents>)self forMode:NSDefaultRunLoopMode];

    [l addEvent:(void *)(gsaddr)desc type:ET_RDESC watcher:(id<RunLoopEvents>)self forMode:NSConnectionReplyMode];

    [l addEvent:(void *)(gsaddr)desc type:ET_RDESC watcher:(id<RunLoopEvents>)self forMode:xWaitMode];
...
+ (void)xSelectionClear:(XSelectionClearEvent *)xEvent
{
    ...
    if (o == nil) {
        ...
        NSDebugLLog(@"Pbs", @"Selection clear for unknown selection - '%s'.", name);
        XFree(name);
        return;
    }
...
+ (void)xPropertyNotify:(XPropertyEvent *)xEvent
{
    ...
    if (o == nil) {
        ...
        NSDebugLLog(@"Pbs", @"Property notify for unknown property - '%s'.", name);
        XFree(name);
        return;
    }
...
+ (void)xSelectionNotify:(XSelectionEvent *)xEvent
{
    ...
    if (o == nil) {
        char *name = XGetAtomName(xDisplay, xEvent->selection);
        NSDebugLLog(@"Pbs", @"Selection notify for unknown selection - '%s'.", name);
        XFree(name);
        return;
    }
    ...
    if (xEvent->requestor != (Window)xAppWin) {
        ...
        NSDebugLLog(@"Pbs", @"Selection (%s) notify - '%s'.", sel_name, pro_name ? pro_name : "None");
        XFree(sel_name);
        if (pro_name)
            XFree(pro_name);
    }
...
+ (void)xSelectionRequest:(XSelectionRequestEvent *)xEvent
{
    ...
    if (o == nil) {
        char *name = XGetAtomName(xDisplay, xEvent->selection);
        NSDebugLLog(@"Pbs", @"Selection request for unknown selection - '%s'.", name);
        XFree(name);
        return;
    }
...
- (void)requestData:(Atom)xType
{
    ...
    if (whenRequested != CurrentTime) {
         ...
         */
        XConvertSelection(xDisplay, [self xPb], xType, [self xPb], xAppWin, whenRequested);
        XFlush(xDisplay);
        ...
        while (XQLength(xDisplay) > 0 && [self waitingForSelection] == whenRequested) {
            XEvent xEvent;
        ...
        while ([self waitingForSelection] == whenRequested) {
            [[NSRunLoop currentRunLoop] runMode:xWaitMode beforeDate:limit];
...
- (NSArray *)availableTypes
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if ((type == XG_UTF8_STRING) || (type == XA_STRING) || (type == XG_TEXT) || (type == XG_MIME_PLAIN)) {
            [types addObject:NSStringPboardType];
        } else if (type == XG_FILE_NAME) {
            [types addObject:NSFilenamesPboardType];
        } else if ((type == XG_MIME_RTF) || (type == XG_MIME_APP_RTF) || (type == XG_MIME_TEXT_RICHTEXT)) {
            [types addObject:NSRTFPboardType];
        } else if (type == XG_MIME_TIFF) {
            [types addObject:NSTIFFPboardType];
        } else if ((type == XG_TARGETS) || (type == XG_TIMESTAMP) || (type == XG_OWNER_OS) || (type == XG_USER) ||
                   (type == XG_HOST_NAME) || (type == XG_HOSTNAME) || (type == XG_MULTIPLE)) {
            // Standard types
        }
        // FIXME: Support more types
        else {
            ...
            NSDebugLLog(@"Pbs", @"Unsupported selection type '%s' available", name);
            XFree(name);
        }
    }
...
- (NSMutableData *)getSelectionData:(XSelectionEvent *)xEvent type:(Atom *)type
{
    int status;
    unsigned char *data;
    long long_offset = 0L;
     ...
     */
    do {
        status = XGetWindowProperty(xDisplay, xEvent->requestor, xEvent->property,
                                    long_offset,  // offset
                                    long_length,
                                    False,  // Aug 2011 - changed to False (don't delete property)
                                    req_type, &actual_type, &actual_format, &number_items, &bytes_remaining, &data);
        ...
        if ((status == Success) && (number_items > 0)) {
            ...
            if (md == nil) {
                md = [[NSMutableData alloc] initWithBytes:(void *)data length:count];
                ...
                if (req_type != actual_type) {
                    ...
                    NSLog(@"Selection changed type from %s to %s.", req_name, act_name);
                    XFree(req_name);
                    XFree(act_name);
                    RELEASE(md);
                    return nil;
...
- (void)xSelectionNotify:(XSelectionEvent *)xEvent
{
    ...
    if (md != nil) {
        ...
        if (actual_type == XG_UTF8_STRING) {
            ...
            s = [[NSString alloc] initWithData:md encoding:NSUTF8StringEncoding];
            if (s != nil) {
                d = [NSSerializer serializePropertyList:s];
                RELEASE(s);
                [self setData:d];
            }
        } else if ((actual_type == XA_STRING) || (actual_type == XG_TEXT) || (actual_type == XG_MIME_PLAIN)) {
            NSString *s;
            NSData *d;

            s = [[NSString alloc] initWithData:md encoding:NSISOLatin1StringEncoding];
            if (s != nil) {
                d = [NSSerializer serializePropertyList:s];
                RELEASE(s);
                [self setData:d];
            ...
            RELEASE(url);
        } else if ((actual_type == XG_MIME_RTF) || (actual_type == XG_MIME_APP_RTF) ||
            ...
            NSDebugLLog(@"Pbs", @"Unsupported data type '%s' from X selection.", name);
            XFree(name);
        }
    }
}
...
- (void)xSelectionRequest:(XSelectionRequestEvent *)xEvent
{
    ...
    XSendEvent(xEvent->display, xEvent->requestor, False, 0L, (XEvent *)&notify_event);
}
...
- (BOOL)xProvideSelection:(XSelectionRequestEvent *)xEvent
{
    ...
    if (xEvent->target == XG_TARGETS) {
    ...
    } else if ((xEvent->target == XG_HOST_NAME) || (xEvent->target == XG_HOSTNAME)) {
        ...
        if (xEvent->property != None) {
            ...
            status = XGetWindowProperty(xDisplay, xEvent->requestor, xEvent->property, 0, 100, False,
                                        AnyPropertyType, &actual_type, &actual_format, &number_items,
                                        &bytes_remaining, (unsigned char **)&multipleAtoms);
            if ((status == Success) && (bytes_remaining == 0) && (actual_format == 32) &&
                (actual_type == XA_ATOM)) {
                int i;
    ...
    } else if ((xEvent->target == XG_COMPOUND_TEXT) && [types containsObject:NSStringPboardType]) {
        ...
        if (d) {
            ...
            status = XmbTextListToTextProperty(xEvent->display, list, 1, XCompoundTextStyle, &textProperty);
            if (status == Success) {
    ...
    } else if (((xEvent->target == XG_UTF8_STRING) || (xEvent->target == XA_STRING) ||
    ...
    } else if ((xEvent->target == XG_FILE_NAME) && [types containsObject:NSFilenamesPboardType]) {
        NSArray *names = [_pb propertyListForType:NSFilenamesPboardType];
        NSString *file = [[names lastObject] stringByStandardizingPath];
        NSURL *url = [[NSURL alloc] initWithScheme:NSURLFileScheme host:@"localhost" path:file];
    ...
    } else if (((xEvent->target == XG_MIME_RTF) || (xEvent->target == XG_MIME_APP_RTF) ||
    ...
    } else if ((xEvent->target == XG_MIME_TIFF) && [types containsObject:NSTIFFPboardType]) {
    ...
    return [self xSendData:data format:format items:numItems type:xType to:xEvent->requestor
         ...
         property:(Atom)property
{
    ...
    if (data != 0 && numItems != 0 && format != 0) {
        ...
        while (appendFailure == NO && pos < numItems) {
            ...
            XChangeProperty(xDisplay, window, property, xType, format, mode, &data[pos * format / 8], maxItems);
            mode = PropModeAppend;
            pos += maxItems;
...
- (Time)xTimeByAppending
{
     ...
     */
    XGetWindowProperty(xDisplay, xAppWin, [self xPb], 0, 0, False, AnyPropertyType, &actualType, &actualFormat, &ni,
                       &ba, &pr);
    if (pr != 0)
        XFree(pr);
    ...
    XChangeProperty(xDisplay, xAppWin, [self xPb], actualType, actualFormat, PropModeReplace, 0, 0);
    XFlush(xDisplay);
...
mimeTypeForPasteboardType(Display *xDisplay, NSZone *zone, NSArray *types)
{
    ...
    for (i = 0; i < count; i++) {
        NSString *mime = [NSPasteboard mimeTypeForPasteboardType:[types objectAtIndex:i]];
...
pasteboardTypeForMimeType(Display *xDisplay, NSZone *zone, Atom *typelist)
{
    ...
    while (*type != None) {
        ...
        if (s) {
            [newTypes addObject:[NSPasteboard pasteboardTypeForMimeType:[NSString stringWithCString:s]]];
            XFree(s);
        }
    }
