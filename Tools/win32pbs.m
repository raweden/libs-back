...
+ (BOOL)initializePasteboard
{
    if (self == [Win32PbOwner class]) {
        wpb = [[Win32PbOwner alloc] initWithOSPb:[NSPasteboard generalPasteboard]];
...
- (id)initWithOSPb:(NSPasteboard *)ospb
{
    ...
    if (RegisterClassEx(&wc)) {
        _hwnd = CreateWindowEx(0, "GNUstepClipboardClass", "GNUstepClipboard", 0, 0, 0, 10, 10, HWND_MESSAGE,
                               (HMENU)NULL, _hinstance, NULL);
    }
...
{
    if ((_hwnd != GetClipboardOwner()) && IsClipboardFormatAvailable(CF_UNICODETEXT)) {
        [_pb declareTypes:[NSArray arrayWithObject:NSStringPboardType] owner:self];
    }
}
...
- (void)provideStringTo:(NSPasteboard *)pb
{
    ...
    if (!IsClipboardFormatAvailable(CF_UNICODETEXT) || !OpenClipboard(_hwnd)) {
        return;
    }
    ...
    if (hglb != NULL) {
        ...
        if (lpwstr != NULL) {
            ...
            len = lstrlenW(lpwstr);
            s = [NSString stringWithCharacters:lpwstr length:len];
...
#define WIN_MSG_QUEUE_FNAME "/dev/windows"
    ...
    [currentRunLoop addEvent:(void *)fdMessageQueue type:ET_RDESC watcher:(id<RunLoopEvents>)self forMode:mode];
#else
    [currentRunLoop addEvent:(void *)0 type:ET_WINMSG watcher:(id<RunLoopEvents>)self forMode:mode];
#endif
}
